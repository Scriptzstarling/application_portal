...
export default function Register() {
  ...
  async function sendOtp(e) {
    ...
    setMessage('');
    try {
      await api('/auth/send-otp', { method: 'POST', body: JSON.stringify({ mobile, username }) });
      setStep('enterOtp');
  ...
  async function verifyOtp(e) {
    ...
    setMessage('');
    try {
      await api('/auth/verify-otp', { method: 'POST', body: JSON.stringify({ mobile, otp, username }) });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-3 sm:px-4">
      <div className="bg-white rounded-2xl shadow-md w-full max-w-md sm:max-w-lg p-6 sm:p-8">
        <h2 className="text-2xl font-semibold text-gray-800 mb-2">Register</h2>
        <p className="text-sm text-gray-500 mb-6">
          Please complete the form with accurate details to register your account.
        </p>
